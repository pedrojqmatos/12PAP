...
Public Class Status
    ...
    Private Sub PictureBox1_Click(sender As Object, e As EventArgs) Handles PictureBox1.Click
        If marca_ja_existe(TextBox1.Text) Then
            MessageBox.Show("Esse tipo de status já existe na tabela!", "aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        Dim cmd As MySqlCommand
        connection.Open()
        cmd = connection.CreateCommand
        cmd.CommandText = "insert into Status(ST_Status)values(@status);"
    ...
    Private Sub PictureBox3_Click(sender As Object, e As EventArgs) Handles PictureBox3.Click
        ...
        connection.Open()
        cmd = connection.CreateCommand()
        cmd.CommandText = "delete from status where ST_Status=@status;"
        cmd.Parameters.AddWithValue("@status", TextBox1.Text)
        cmd.ExecuteNonQuery()
        connection.Close()
        lista()
    ...
    Private Sub TextBox2_TextChanged(sender As Object, e As EventArgs) Handles TextBox2.TextChanged
        ...
        Dim ds As New DataSet
        ds.Tables.Add(dt)
        Dim da As New MySqlDataAdapter("SELECT * FROM status where ST_Status Like '%" & TextBox2.Text & "%' Order By ST_Status", connection)
        da.Fill(dt)
        Dim coluna As DataRow
        For Each coluna In dt.Rows
            ListView1.Items.Add(coluna.Item(0))
    ...
    Private Function marca_ja_existe(marca As String) As Boolean
        Dim cmd As MySqlCommand
        cmd = connection.CreateCommand()
        cmd.CommandText = "Select count(*) from status where ST_Status = @status"
        cmd.Parameters.AddWithValue("@status", marca)
